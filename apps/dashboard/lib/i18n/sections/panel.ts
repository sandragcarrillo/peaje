export const panel = {
  es: {
    eyebrowNegocio: 'NEGOCIO',
    consola: 'CONSOLA',
    navPortal: 'PORTAL DEV',
    colUltimaVez: 'Última vez',
    colGasto: 'Gasto total',
    ordenGasto: 'Orden: mayor gasto',
    finLedger: (n: number) => `Fin del ledger — ${n} de ${n} agentes reportados`,
    ...
    scoreCol2: 'Lo que el kit desbloquea',
    scoreChecksOk: (n: number) => `${n} checks pasan`,
    scoreChecksPend: (n: number) => `${n} por atacar`,
    subDisponible: 'Listo para retirar',
    subRequests: 'Todo el tiempo',
    subRevenue: 'Vía MPP · 402',
    histLabel: 'Histórico',
    hoyLabel: 'Hoy',
    filtro402: 'Filtro: pagos 402 liquidados',
    mostrando: (n: number, m: number) => `Mostrando ${n} de ${m} pagos recientes`,
    setupListo: '[✓] Listo',
    setupPendiente: '[ ] Te falta esto',
    pasoLabel: (n: number) => `Paso 0${n}`,
    sidebarNuevaRuta: 'NUEVA RUTA',
    scoreMejora: (antes: number, ahora: number) =>
      `Peaje está conectado: tu sitio pasó de ${antes} a ${ahora} para agentes.`,
    scoreMejoraNota: (delta: number) =>
      `+${delta} puntos desde que llegaste. El primer scan queda guardado como tu punto de partida.`,
    sidebarCambiar: 'CAMBIAR NEGOCIO',
    estadoActivo: 'ACTIVO',
    estadoRutas: 'RUTAS',
    rieles: 'RIELES',
    // layout
  ...
  en: {
    eyebrowNegocio: 'BUSINESS',
    consola: 'CONSOLE',
    navPortal: 'DEV PORTAL',
    colUltimaVez: 'Last seen',
    colGasto: 'Lifetime spend',
    ordenGasto: 'Sort: lifetime spend',
    finLedger: (n: number) => `End of ledger — ${n} of ${n} agents reported`,
    hace: (v: string) => `${v} ago`,
    ...
    scoreCol2: 'What the kit unlocks',
    scoreChecksOk: (n: number) => `${n} checks pass`,
    scoreChecksPend: (n: number) => `${n} to attack`,
    subDisponible: 'Ready to withdraw',
    subRequests: 'All time',
    subRevenue: 'Via MPP · 402',
    histLabel: 'Historical',
    hoyLabel: 'Today',
    filtro402: 'Filter: HTTP 402 settled',
    mostrando: (n: number, m: number) => `Showing ${n} of ${m} recent payments`,
    setupListo: '[✓] Done',
    setupPendiente: '[ ] Action needed',
    pasoLabel: (n: number) => `Step 0${n}`,
    sidebarNuevaRuta: 'NEW ROUTE',
    scoreMejora: (antes: number, ahora: number) =>
      `Peaje is connected: your site went from ${antes} to ${ahora} for agents.`,
    scoreMejoraNota: (delta: number) =>
      `+${delta} points since you arrived. Your first scan is saved as the starting point.`,
    sidebarCambiar: 'SWITCH BUSINESS',
    estadoActivo: 'ACTIVE',
    estadoRutas: 'ROUTES',
    rieles: 'RAILS',
    // layout
